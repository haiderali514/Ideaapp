...
export const UncategorizedChatList: React.FC<UncategorizedChatListProps> = (props) => {
    const { isExpanded, onToggle, chats, activeChatId, onSelectChat, isSidebarExpanded, onDeleteChat, onOpenRenameModal, onOpenMoveModal } = props;
                        ...
                        <div className="project-folder-actions" ref={activeChatMenu === chat.id ? activeMenuRef : null}>
                            ...
                            {activeChatMenu === chat.id && (
                                <div className="options-dropdown">
                                    <button className="dropdown-item" onClick={(e) => { e.stopPropagation(); handleAction(() => onOpenMoveModal(chat))}}>Add to project</button>
                                    <button className="dropdown-item" onClick={(e) => { e.stopPropagation(); handleAction(() => onOpenRenameModal(chat))}}>Rename</button>
                                    <button className="dropdown-item delete" onClick={(e) => { e.stopPropagation(); handleAction(() => onDeleteChat(chat))}}>Delete chat</button>
                                </div>
                            )}
                        </div>
                    )}
